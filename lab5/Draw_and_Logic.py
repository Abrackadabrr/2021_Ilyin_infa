...
class DrawMaster:
    ...
    def draw_end_level(self, is_end):
        """
        Проверяет, кончился ли уровень и если да - рисует кой чево
        :param is_end: кончился ли уровень
        :return: is_end
        """
        if is_end:
            self.print_text("Цель уничтожена; количество выстрелов: " + str(self.amount_of_all_balls), COLORS[0],
                            (SCREEN_X/6, SCREEN_Y/4), 40)
        return is_end
    ...
    def draw(self, time, score, is_level_end=False):
        """
        НЕСТАНДАРТНЫЙ draw
        ...
        :param is_level_end: булевая переменная отвечающая за "конец раунда"
        """
        for figure in self.massive_of_acting_figures:
            figure.update(time)
            figure.draw()
        ...
        self.draw_end_level(is_level_end)
