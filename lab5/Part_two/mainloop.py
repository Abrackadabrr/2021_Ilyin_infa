...
def the_end(score):
    ...
    while pause_time < 3000:
        ...
        draw_master.draw(pause_time, score)
        draw_master.draw_end_of_game(score)
...
while not finished:
    ...
    while not end_of_level:
        ...
        draw_master.draw_remaining_time(game_time)
        end_of_level, score_of_one_level, finished = game_logic.update_game_procces(game_time)
        pygame.display.update()
        if end_of_level and not finished:
            global_score += score_of_one_level
            pause(global_score)
        if end_of_level and finished:
            global_score += score_of_one_level
            the_end(global_score)
